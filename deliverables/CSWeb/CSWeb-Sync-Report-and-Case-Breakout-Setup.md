...
```
*/5 * * * * flock -n /tmp/csweb-process-cases.lock docker exec lamp-php8 sh -c "cd /var/www/html/csweb && php bin/console csweb:process-cases" >> /var/log/csweb-process-cases.log 2>&1
```
`flock` prevents overlap (each run lasts up to ~5 min by design). Logrotate at `/etc/logrotate.d/csweb-process-cases` (weekly × 4, compressed). First cron run verified 17:55 UTC.
