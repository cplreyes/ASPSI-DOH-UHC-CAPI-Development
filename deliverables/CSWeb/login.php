...
function intended_path(): string {
    ...
    foreach ($cands as $c) {
        /* The portal on capi.asiansocial.org shares this sign-in (2026-07-28):
         * allow a full-URL return to THAT origin only — exact prefix match,
         * sane path chars, no scheme/host smuggling. Anything else falls
         * through to the /docs-relative rule below. */
        $capi = 'https://capi.asiansocial.org/';
        if (strpos($c, $capi) === 0) {
            $path = substr($c, strlen($capi) - 1);
            if (preg_match('#^/[A-Za-z0-9._~!$&\x27()*+,;=:@%/?&=-]*$#', $path)
                && strpos($path, '//') === false
                && strpos($path, '/docs/login.php') !== 0
                && strpos($path, '/docs/auth/') !== 0) {
                return $capi . ltrim($path, '/');
            }
        }
