...
>
> **Carl must own or control a domain and be able to add a DNS `A` record before execution day.** A subdomain of any domain ASPSI already controls is fine (e.g. `csweb.aspsi-domain.org`). If no domain exists, registering one is the first action — it is not optional, and it is not something the on-VPS Claude can do.
>
...
|---|---|---|---|
| `<your-domain>` | The FQDN CSWeb will live at | Carl (Part 0.1) | `csweb.aspsi-domain.org` |
...
### 2.8 Reboot if the kernel was updated
...
```bash
sudo sed -i 's/<your-domain>/csweb.aspsi-domain.org/' /etc/apache2/sites-available/csweb.conf
```
