...
| Version | Date | Change |
|---|---|---|
| 0.8 | 2026-08-18 | **F3 Q18 income-bracket recode (Aug-17 rewrite, R16 -- reverses R4/#631).** `Q18_INCOME_BRACKET`'s value set moved from 11 round-50k bands (`01`-`11`, DK=`99`/RF=`98`) to the Aug-17 paper's 7 PSA income-class bands (`1`-`7`; boundaries 12,030 / 24,060 / 48,120 / 84,210 / 144,360 / 240,600 PHP/month), DK=`8`/RF=`9`, 1-digit codes (no zero-fill, was 2-digit zero-filled). A declared data-shape break (F3 -> 4.0.0) -- paper codes win per Carl's ruling; the old 11-band codes **no longer exist**. **Downstream impact:** (a) any prior extract/analysis keyed on the old `01`-`11`/`98`/`99` codes must re-map through this table before merging with post-Aug-17 data -- the two code schemes are NOT compatible (e.g. old `03` = "100,000-149,999" vs new `3` = "PhP24,061 to PhP48,120" cover different ranges); (b) the amount<->bracket consistency PROC and the `Q29_SEC_CLASS` socio-economic-class SOFT cross-check (both in `generate_apc.py`) were re-bounded to match -- see `aug17-approved-divergences.md` (F3 | Q18) for the full cascade; (c) `Q18_INCOME_AMOUNT` (the free-entry peso figure, `-98`/`-99` DK/RF sentinels) is unchanged -- only the ticked-bracket companion field's coding moved. Source of truth = `deliverables/CSPro/F3/generate_dcf.py` / `generate_apc.py`. Task 1.4, 2026-08-18. |
