// Bikol bundle — app CHROME is English-fallback (DRAFT, pending ASPSI QC).
// The Bikol SURVEY CONTENT (questions, choices, sections) is fully wired and
// lives in spec/translations/bcl.json. App-furniture strings (buttons, sync,
// errors) are not in the ASPSI questionnaire docs, so they show English until QC.
import type { EnBundle } from './en';

export const bcl: EnBundle = {
  chrome: {
  ...
  consent: {
    ...
    // with the Aug-17 paper and no cleared bcl translation exists yet.
    infoStudy:
