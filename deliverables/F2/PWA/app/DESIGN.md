...
| 2026-08-17 | Government logo strip + PSA/SJREB clearance block added to the masthead (Carl, per the RAs' reference layout) | Deliberate, approved bend of the "not a government form" anchor: F2 was the only instrument in the suite showing neither the DOH/Bagong Pilipinas/ASPSI logo strip nor its PSA SSRCS clearance stamp — the CSPro instruments carry both on their cover/ICF screens, and the presentation expectation applies to the cleared instrument set as a whole. Implementation: `components/chrome/GovBrand.tsx` — the CAPI cover's composed logo strip (`gov-logos.png`) + ASPSI mark at `h-7`/`h-9`, and a two-line `font-mono text-[10px] muted` clearance block (regulatory metadata, identical in all locales, NOT i18n'd). Header switched to `flex-wrap`; on `<md` the clearance wraps to its own full-width row so the stamp stays visible on phones. Clearance suffix `-02` inferred from the PSA series documented in `deliverables/CSPro/icf_content.py` (−01 Facility Head, −03 Patient, −04 Household); flagged for ASPSI veto. No palette or token values changed. |
| 2026-08-20 | Masthead logo strip enlarged to 100px and moved out of the chrome-controls cluster (#1281) | UAT R7 reviewers reported the wordmarks inside the strip were unreadable and asked for a ~100px minimum. At the shipped `h-7`/`h-9` the text inside `gov-logos.png` rendered around 4px tall, so the complaint is legitimate rather than a taste preference. **100px is also the ceiling, not an arbitrary target**: the source assets are `gov-logos.png` 520x107 and `aspsi.png` 346x214, so rendering past ~107px upscales and blurs — which would defeat the purpose. Sizes are now `h-14` / `sm:h-20` / `md:h-[100px]`: the reviewers tested on a Windows laptop, so md+ meets their stated minimum at essentially 1:1 for the gov strip, while phones keep a smaller (but still 2x) strip because a 100px strip cannot share a row with the language switcher and buttons. The strip also left the controls `<div>` and became its own header flex item, wrapping full-width and centred below `sm` — otherwise it pushed the chrome controls off the row. This deepens the 2026-08-17 bend of the "not a government form" anchor: the masthead is now visually heavier than the Verde Manual would choose unprompted, accepted because it is a cleared-instrument presentation requirement from the client's reviewers. Alternative offered on the ticket if they want the chrome back to compact: carry the full-size strip on the consent/cover screen only, as the CSPro instruments do. No palette or token values changed. |
| 2026-08-20 | Masthead logo strip replaced with the CAPI cover's actual composed asset — ASPSI moved from last to second | Carl: "fix the F2 logo arrangement just like the other instruments." F2's mark order did not match F1/F3/F4. #1190 (Shan, ASPSI, 2026-08-11) fixes the brand-book sequence for the cleared instrument set as **DOH Seal → ASPSI → Bagong Pilipinas → Bawat Buhay Mahalaga**, with ASPSI second in the "attached agency" slot; F2 was rendering DOH → Bagong Pilipinas → Bawat Buhay Mahalaga → ASPSI. **Root cause, and a correction to the 2026-08-17 row above:** that entry describes `gov-logos.png` as "the CAPI cover's composed logo strip". It was not. `gov-logos.png` was byte-identical (md5 `7f3cba68…`) to `deliverables/CSPro/logos/cover_logos_doh3.png` — the **pre-composition, DOH-only input** to `compose_cover_logos.py`, which is precisely the file that script splits apart in order to insert ASPSI into second place. F2 took the input rather than the output and appended ASPSI to the end, so the agency landed in the wrong slot. Fix: `src/assets/brand/cover-logos.png` is now a copy of `deliverables/CSPro/cover_logos.png` — the same 666×107 composed strip the three CSPro covers embed as a data-URI — rendered as a **single** `<img>`; `gov-logos.png` and `aspsi.png` are deleted so the pre-composition input cannot be picked up again. Sizes are unchanged (`h-14` / `sm:h-20` / `md:h-[100px]`) and 107px remains the no-upscale ceiling; as one image the strip is slightly narrower at md+ (~622px vs ~660px for the two it replaces), so the header row does not regress. Alt text now names all four marks in order. No palette or token values changed. Provenance rule recorded in `GovBrand.tsx`: change the marks by editing `deliverables/CSPro/logos/`, re-running `compose_cover_logos.py`, and re-copying the output — never re-cut or re-split the strip by hand. |
