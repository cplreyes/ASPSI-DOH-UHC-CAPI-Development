...
function _assertStagingOnly() {
  ...
  try {
    ...
    if (name.toLowerCase().indexOf("staging") === -1) {
      throw new Error(
        "Refusing to seed demo data: this does not look like a staging " +
          'sheet (name="' +
          name +
          '", PROP_ENV unset). To override for ' +
    ...
    throw new Error(
      "Refusing to seed demo data: cannot verify staging environment.",
    );
  }
}
...
function _seedHCWs(ss) {
  ...
  if (!sh)
    throw new Error("F2_HCWs sheet not found — run runAllMigrations() first");
  var headers = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0];
...
function _seedResponses(ss) {
  ...
  if (lastRow > 1) {
    ...
    for (var k = 0; k < data.length; k++)
      existing[String(data[k][hcwIdx])] = true;
  }
  var added = 0,
    skipped = 0;
  var hcws = _demoHCWs();
  ...
  for (var i = 0; i < hcws.length; i++) {
    ...
    var row = headers.map(function (col) {
      if (col === "submission_id")
        return "DEMO-SUB-" + h.hcw_id.replace("DEMO-HCW-", "");
      if (col === "client_submission_id")
        return "DEMO-CSID-" + h.hcw_id.replace("DEMO-HCW-", "") + "-1";
      ...
      if (col === "submission_lat")
        return fac ? fac.lat + (Math.random() - 0.5) * 0.005 : "";
      if (col === "submission_lng")
        return fac ? fac.lng + (Math.random() - 0.5) * 0.005 : "";
      if (col === "source_path")
        return h.status === "paper_encoded" ? "paper_encoded" : "self_admin";
      if (col === "encoded_by")
        return h.status === "paper_encoded" ? "carl_admin" : "";
      if (col === "encoded_at")
        return h.status === "paper_encoded" ? submittedAt : "";
      return "";
    });
    sh.appendRow(row);
    added++;
...
function _seedDLQ(ss) {
  ...
  var entry = {
    ...
    reason:
      "E_SPEC_DRIFT: client submitted spec_version=2026-04-08 below min_accepted=2026-04-17-m1; tablet needs to fetch latest config and resubmit.",
    payload_json: JSON.stringify({
      ...
      app_version: "1.2.3",
      values_summary:
        "Section A complete; submission rejected at gate before sheet write.",
    }),
  };
  if (existing[entry.client_submission_id]) {
    ...
    var row = headers.map(function (col) {
      return entry[col] != null ? entry[col] : "";
    });
    sh.appendRow(row);
    added = 1;
  }
...
function _seedFiles(ss) {
  ...
  if (!sh)
    throw new Error(
      "F2_FileMeta sheet not found — run runAllMigrations() first",
    );
  var headers = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0];
  ...
  var files = [
    {
      ...
      description:
        "Q1 fieldwork plan — facility coverage + enumerator assignments.",
      deleted_at: "",
    },
    {
      ...
      description:
        "Sample facility list with PSGC region/province/city/barangay cols.",
      deleted_at: "",
    },
  ];
  for (var i = 0; i < files.length; i++) {
    var f = files[i];
    if (existing[f.file_id]) {
      skipped++;
      continue;
    }
    var row = headers.map(function (col) {
...
function _seedAuditEvents(ss) {
  ...
  var events = [
    {
      ...
      event_payload_json: JSON.stringify({
        reason: "HCW lost device; reissue requested via Viber",
      }),
      client_ip_hash: "",
  ...
  for (var i = 0; i < events.length; i++) {
    var e = events[i];
    if (existing[e.audit_id]) {
      skipped++;
      continue;
    }
    var row = headers.map(function (col) {
      return e[col] != null ? e[col] : "";
    });
    sh.appendRow(row);
    added++;
  }
...
function _demoJti(hcwId, sequence) {
  ...
  return (
    "demo-jti-" + hcwId.replace("DEMO-HCW-", "").toLowerCase() + "-" + sequence
  );
}
