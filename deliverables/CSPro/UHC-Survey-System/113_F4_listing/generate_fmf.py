"""generate_fmf.py — emit F4Listing.fmf in canonical CSPro 8.0 layout.

Parallel to 110_F3_listing/generate_fmf.py — same single-column auto-
layout via shared.form_layout_engine, BOM-prefixed, [Form] / [Level] /
[Group] / [Field] / [Text] structure.
...
LEVEL_NAME  = "F4LISTING_LEVEL"
LEVEL_LABEL = "F4Listing Level"
FF_NAME     = "F4LISTING_FF"
FF_LABEL    = "F4Listing"
DCF_PATH    = HERE / "F4Listing.dcf"
...
def main():
    if not DCF_PATH.exists():
        raise SystemExit(f"F4 listing DCF not found at {DCF_PATH} — run generate_dcf.py first")
    ...
    for form_index_zero_based, (_form_name, form_label, group_name, items) in enumerate(forms):
        ...
        # Occurring records (REC_LISTING_BRGY_FRAME) need Max>1 on their
        # [Group] so CSPro Designer wires the roster navigation. Read
        # the max occurrences from the source record if present; otherwise
        # default to 1.
        max_occ = 1
    ...
    body = "".join(parts)
    (HERE / "F4Listing.fmf").write_text("﻿" + body, encoding="utf-8")
    item_total = sum(len(f[3]) for f in forms)
    print(f"wrote F4Listing.fmf ({len(forms)} forms, {item_total} items, BOM-prefixed)")
