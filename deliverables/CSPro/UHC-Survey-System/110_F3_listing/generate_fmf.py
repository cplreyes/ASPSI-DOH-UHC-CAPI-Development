"""generate_fmf.py — emit PatientListing.fmf in canonical CSPro 8.0 layout.

Parallel to 107_F1/generate_fmf.py — same single-column auto-layout via
shared.form_layout_engine, BOM-prefixed, [Form] / [Level] / [Group] /
[Field] / [Text] structure.
...
LEVEL_NAME  = "PATIENTLISTING_LEVEL"
LEVEL_LABEL = "PatientListing Level"
FF_NAME     = "PATIENTLISTING_FF"
FF_LABEL    = "PatientListing"
DCF_PATH    = HERE / "PatientListing.dcf"
...
def all_id_items(level):
    ...
    multi-item case-ID block and the single-item shape used by the
    commit-1 stub. CSPro 8.0 accepts both."""
    ids = level.get("ids", {})
...
def main():
    if not DCF_PATH.exists():
        raise SystemExit(f"Listing DCF not found at {DCF_PATH} — run generate_dcf.py first")
    ...
    for form_index_zero_based, (_form_name, form_label, group_name, items) in enumerate(forms):
        ...
        # Occurring records (REC_RANDOM_INTERVAL_LOG, REC_PATIENT_ROSTER)
        # need Max>1 on their [Group] so CSPro Designer wires the roster
        # navigation. Read the max occurrences from the source record if
        # present; otherwise default to 1.
        max_occ = 1
        # Look up the source record by index (forms[0] is IDS, so records[i-1])
    ...
    body = "".join(parts)
    (HERE / "PatientListing.fmf").write_text("﻿" + body, encoding="utf-8")
    item_total = sum(len(f[3]) for f in forms)
    print(f"wrote PatientListing.fmf ({len(forms)} forms, {item_total} items, BOM-prefixed)")
