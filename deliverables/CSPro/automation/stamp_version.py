...
Usage:
  ...
  py stamp_version.py bump F3 --notes "fixed Q45 skip"   # bump + record a release-note bullet

Every bump/set also regenerates ../RELEASE-NOTES.md (via release_notes.py) -- the
per-instrument release-notes + versioning monitor (Keep a Changelog format). Pass
--notes "text" (keep it last on the command line) to record a human bullet for the
new version; it is typed by bump kind (patch->Fixed, minor->Added, major->Changed)
unless --type added|changed|fixed|removed|security overrides, and --breaking flags a
data-shape break. Without --notes the entry falls back to the commit subject.
...
def main():
    ...
    if cmd in ("bump", "set"):
        # fourth stamped surface: RELEASE-NOTES.md rides every version change
        try:
            import release_notes
            if "--notes" in rest:
                txt = rest[rest.index("--notes") + 1]
                if "--type" in rest:
                    ntype = rest[rest.index("--type") + 1]
                elif "--major" in rest:
                    ntype = "Changed"
                elif "--minor" in rest:
                    ntype = "Added"
                    ...
                    ntype = "Fixed"
                for key in keys:
                    release_notes.add_note(key, v[key]["version"], txt,
                                           ntype, "--breaking" in rest)
            release_notes.regen()
        except Exception as e:
            print(f"[notes] ! RELEASE-NOTES update failed: {e}")
            ok = False
