...
def main():
    ...
    fixed_files = {}  # (instrument, locale) -> dict (scoped, mutated)
        ...
        for loc in LOCALES:
            ...
            for key in item_keys:
                ...
                if fired:
                    ...
                    if apply_mode and auto_apply:
                        if fix_value is not None:
                            mutated[key] = fix_value
                        else:
                            del mutated[key]

            if apply_mode:
                fixed_files[(ins, loc)] = mutated
    ...
    if apply_mode:
        for (ins, loc), mutated in fixed_files.items():
            out_path = args.out / ins / "translations" / f"{loc}.json"
            out_path.parent.mkdir(parents=True, exist_ok=True)
            with open(out_path, "w", encoding="utf-8", newline="\n") as f:
                json.dump(mutated, f, ensure_ascii=False, indent=2)
                f.write("\n")
