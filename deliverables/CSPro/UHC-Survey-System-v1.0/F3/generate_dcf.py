...
generate_dcf.py — F3 Patient Survey CSPro Data Dictionary generator
                  (UHC Survey System v1.0 rebuild — phase 2).

Emits PatientSurvey.dcf in CSPro 8.0 JSON dictionary format. 178 numbered
items across sections A-L.
...
v1.0 differences from the predecessor build:
  ...
  - F3 -> F1 linkage is the shared 9-digit prefix REGION+PROVINCE+CITY_MUN+
    FACILITY (the first 9 digits of the case ID). The separate F3_FACILITY_ID
    data item is RETIRED in v1.0.
  - The facility's region/province/city are case-ID items, so PATIENT_GEO_ID
    carries only the facility barangay + name/address and the patient's home
    location (P_ prefix).

Question content (Q1-Q178, sections A-L) is ported verbatim from the
Apr 20 2026 Annex F3 questionnaire build. Item names are unchanged so
F3-Skip-Logic-and-Validations.md references stay valid.
...
from shared.value_sets import SATISFACTION_5PT
...
# ============================================================
# FIELD CONTROL — shared + patient-type extras
# ============================================================
...
def build_f3_field_control():
    extra = [
        ...
        # number back to the paper patient-listing line.
...
# GEOGRAPHIC RECORD
# ============================================================
# Facility region/province/city/number are case-ID items (the 9-digit
# RR-PP-MMM-FF prefix IS the F3->F1 linkage — no separate F3_FACILITY_ID).
# This record carries the facility barangay + name/address and the patient's
# home location. Patient-home PROVINCE/CITY/BARANGAY value sets cascade at
# runtime in the .apc (phase 4); P_REGION_CODE gets the static region set.

def build_patient_geo():
    items = [
        numeric("CLASSIFICATION", "Classification", length=1,
                value_set_options=[("UHC IS", "1"), ("Non-UHC IS", "2")]),
        numeric("BARANGAY_CODE",
                "Facility barangay code (PSA 1Q 2026 PSGC pos 8-10)",
                length=3, zero_fill=True),
        alpha("FACILITY_NAME",    "Facility Name",    length=100),
        alpha("FACILITY_ADDRESS", "Facility Address", length=200),
        numeric("P_REGION_CODE",
                "Patient home — region code (PSGC pos 1-2)",
                length=2, zero_fill=True,
                value_set_options=load_region_value_set()),
        numeric("P_PROVINCE_HUC_CODE",
                "Patient home — province/HUC code (PSGC pos 3-4)",
                length=2, zero_fill=True),
        numeric("P_CITY_MUNICIPALITY_CODE",
                "Patient home — city/municipality code (PSGC pos 5-7)",
                length=3, zero_fill=True),
        numeric("P_BARANGAY_CODE",
                "Patient home — barangay code (PSGC pos 8-10)",
                length=3, zero_fill=True),
    ]
...
# ============================================================
# Section C. Awareness on Universal Health Care (UHC) (Q35-Q37)
# ============================================================
...
# ============================================================
# Section D. PhilHealth Registration and Health Insurance (Q38-Q52)
# ============================================================
...
# ============================================================
# Section F. Patient's Health-Seeking Behavior (Q83-Q87)
# ============================================================
...
def build_section_f():
    ...
    Q87_OTHER_ACTIONS = [
        ...
        # O02 text compressed — the verbatim form overflowed CSPro's 255-char
        # label cap once select_all prepends the Q87 stem (was 305 chars).
        ("Sought alternative care (non-formal care — e.g. reflexology, acupuncture, "
         "massage therapy, herbal medicine)",                                                   "2"),
...
# ============================================================
# Section J. Satisfaction on Amenities and Medical Care (Q131-Q144)
# ============================================================
...
# ============================================================
# Section K. Access to Medicines (Q145-Q161)
# ============================================================
...
# ============================================================
# Section L. Experiences and Satisfaction on Referrals (Q162-Q178)
# ============================================================
...
def build_section_l():
    Q163_CARE_TYPE = [
        ...
        # O06 text compressed — the verbatim form overflowed CSPro's 255-char
        # label cap once select_all prepends the Q163 stem (was 270 chars).
        ("Special therapy visits (rehabilitation — e.g. occupational, physical, and "
         "speech/language therapy, psychosocial rehab, prosthetics/orthotics)",                "06"),
...
def build_f3_facility_capture():
    ...
    return record("REC_FACILITY_CAPTURE", "Facility GPS Capture", "Z",
                  items=gps_fields(prefix="FACILITY_"))


def build_f3_patient_home_capture():
    """GPS metadata for the patient's home (distinct from facility GPS)."""
    return record("REC_PATIENT_HOME_CAPTURE", "Patient Home GPS Capture", "Y",
                  items=gps_fields(prefix="P_HOME_"))


def build_f3_case_verification():
    """Single verification photo per case."""
    return record("REC_CASE_VERIFICATION", "Case Verification Photo", "X",
...
def build_id_items():
    """The 12-digit RR-PP-MMM-FF-CCC level ID block. The RR-PP-MMM-FF prefix is
    the F3->F1 facility linkage; CASE_SEQ is the per-facility patient sequence.
    REGION_CODE value set is populated from the PSA 1Q 2026 PSGC region list."""
    id_items = build_id_block("Patient Survey")
...
def build_dictionary_f3():
    ...
    return build_dictionary("PATIENTSURVEY_DICT", "PatientSurvey",
                            build_id_items(), records)


def main():
    out_path = Path(__file__).parent / "PatientSurvey.dcf"
    write_dcf(build_dictionary_f3(), out_path)
