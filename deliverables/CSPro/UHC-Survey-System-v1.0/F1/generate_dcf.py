...
generate_dcf.py — F1 Facility Head Survey CSPro Data Dictionary generator
                  (UHC Survey System v1.0 rebuild — phase 1).

Emits FacilityHeadSurvey.dcf in CSPro 8.0 JSON dictionary format.
...
v1.0 differences from the predecessor build:
  ...
  - REGION_CODE / PROVINCE_HUC_CODE / CITY_MUNICIPALITY_CODE are ID items, so
    the geographic record (HEALTH_FACILITY_GEO) carries only the descriptors
    that are NOT part of the case ID: classification, barangay, facility
    name/address, latitude/longitude.
  - Shared helpers live in the v1.0 shared/ package.

Question content (Q1-Q166, sections A-H) is ported verbatim from the
Designer-signed-off F1 build (E2-F1-010, 2026-05-04). Item names are unchanged
so F1-Skip-Logic-and-Validations.md references stay valid.

Authority sources (in priority order):
  1. Annex F1 Facility Head Survey Questionnaire, UHC Year 2 (printed)
...
from shared.value_sets import SEX, FREQUENCY, WHY_DIFF_OPTIONS
from shared.case_id import build_id_block
from shared.psgc import load_region_value_set
from shared.gps_photo import build_capture_record


# ============================================================
# 1. CLOSED DESIGN DECISIONS (E2-F1-009b — final defaults)
# ============================================================
# These six items were resolved under E2-F1-009b; the values below are final.
# The PENDING_DESIGN_* names are kept for grep-ability only — treat as closed.

# (1) Q63 ACCRED_WAIT — printed text says "days", bucket labels are "months".
#     Final: keep month buckets.
Q63_USE_DAY_BUCKETS = False

# (2) Secondary data structure — final: emit empty stub records so the
#     dictionary opens, with no items inside (structure still upstream-owned).
SECONDARY_DATA_AS_STUBS = True

# (3) NBB — already two UHC9 items (Q40 NBB, Q41 ZBB). Final: do not also
#     split by facility tier.
NBB_SPLIT_BY_TIER = False

# (4) Q31 EMR_USE — final: "Not applicable" routes to Q35 like the other UHC9
#     NA branches (encoded in PROC at phase 4, not in the dictionary).
Q31_NA_SKIPS = True

# (5) Q166 PD_NURSES — printed nurse list omits "Clinical audits" and
#     "Surgical audits". Final: nurse list excludes audits.
Q166_NURSES_INCLUDE_AUDITS = False

# (6) Q121 DOH_LIC_DIFFICULT — final: single value set + skip on
#     Q132/Q133/Q134, not a facility-type dynamic value set.
...
# 2. GEOGRAPHIC RECORD
# ============================================================
# region / province / city / facility-number are case-ID items (see
# shared/case_id.py). This record carries only the remaining descriptors.
# BARANGAY_CODE is the 3-digit PSGC slice (positions 8-10); its value set is
# populated at runtime by the PSGC cascade in the .apc (phase 4).

def build_facility_geo():
    items = [
        numeric("CLASSIFICATION", "Classification", length=1,
    ...
    return record("HEALTH_FACILITY_GEO",
                  "Health Facility Geographic Identification", "B", items)


# ============================================================
# 3. SECTION A — Facility Head Profile (Q1-Q6)
# ============================================================
...
def build_section_a():
    ...
    items = [
        # Respondent contact block — lives with the facility-head profile.
...
# ============================================================
# 4. SECTION B — Facility Profile (Q7-Q8)
# ============================================================
...
# ============================================================
# 5. SECTION C — UHC Implementation (Q9-Q50)
# ============================================================
...
def build_section_c():
    ...
    # Q43 — verbatim text exceeds CSPro's 255-char label limit; ward-allocation
    # parenthetical is enumerator context, not the question, so dropped here.
...
# ============================================================
# 6. SECTION D — YAKAP / Konsulta (Q51-Q100)
# ============================================================
...
def build_section_d():
    ...
    # Q63 — closed decision: month buckets (printed text says days; see (1) above).
    Q63_BUCKETS = Q62_TRANCHE_INTERVAL
    ...
    # Q66-Q74 = nine "why difficult" select-alls, gated on Q65 in PROC (phase 4).
    Q66_74_TOPICS = [
    ...
    items.append(numeric("Q87_REGISTERED_PATIENTS",
                         ...
                         length=7))
    # Q88 — verbatim text is 448 chars, over CSPro's 255-char label limit;
    # PhilHealth tranche-mechanics middle compressed to a parenthetical.
...
# ============================================================
# 7. SECTION E — BUCAS / GAMOT (Q101-Q117)
# ============================================================
...
# ============================================================
# 8. SECTION F — DOH Licensing (Q118-Q134)
# ============================================================
...
# ============================================================
# 9. SECTION G — Service Delivery (Q135-Q162)
# ============================================================
...
# ============================================================
# 10. SECTION H — Human Resources (Q163-Q166)
# ============================================================
...
def build_section_h():
    ...
    # Q166 — closed decision (5): nurse list omits "Clinical/Surgical audits".
...
# ============================================================
# 11. SECONDARY DATA STUBS
# ============================================================

def build_secondary_data_stubs():
    """Closed decision (2): secondary-data structure is upstream-owned. Emit
    empty stub records so the dictionary opens; items added when the structure
    is finalised."""
    if not SECONDARY_DATA_AS_STUBS:
        raise NotImplementedError("Non-stub secondary data structure not yet decided")
    return [
        record("SEC_HOSP_CENSUS", "Secondary Data — Hospital Census 6mo (structure TBD)", "J", []),
        record("SEC_HCW_ROSTER",  "Secondary Data — HCW Full/Part-time Roster (structure TBD)", "K", []),
        record("SEC_YK_SERVICES", "Secondary Data — YAKAP Services Availability (structure TBD)", "L", []),
        record("SEC_LAB_PRICES",  "Secondary Data — Lab Procurement vs Charged Prices (structure TBD)", "M", []),
    ]


# ============================================================
# 12. ASSEMBLE THE DICTIONARY
# ============================================================

def build_id_items():
    """The 12-digit RR-PP-MMM-FF-CCC level ID block, with the REGION_CODE
    value set populated from the PSA 1Q 2026 PSGC region list."""
    id_items = build_id_block("Facility Head Survey")
    region_vs = id_items[0]["valueSets"][0]
    region_vs["values"] = [
        {"labels": [{"text": name}], "pairs": [{"value": code}]}
        for name, code in load_region_value_set()
    ]
    return id_items


def build_dictionary_f1():
    records = [
        record("FACILITYHEADSURVEY_REC", "FacilityHeadSurvey Record", "1", []),
        build_field_control("F1", date_label_entity="the Facility"),
        build_facility_geo(),
        build_capture_record("REC_FACILITY_CAPTURE",
                             "Facility GPS and Verification Photo", "Z",
                             gps_prefix="FACILITY_", photo_prefix=""),
        build_section_a(),
        build_section_b(),
        build_section_c(),
        build_section_d(),
    ...
    return build_dictionary("FACILITYHEADSURVEY_DICT", "FacilityHeadSurvey",
