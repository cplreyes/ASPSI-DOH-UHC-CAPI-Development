...
generate_dcf.py — F4 Household Survey CSPro Data Dictionary generator
                  (UHC Survey System v1.0 rebuild — phase 3).

Emits HouseholdSurvey.dcf in CSPro 8.0 JSON dictionary format. 202 numbered
items across sections A-Q, with a repeating household roster (max_occurs=20).

v1.0 differences from the predecessor build:
  - Case ID is the 12-digit RR-PP-MMM-FF-CCC block (5 separate level ID items
    from shared/case_id.py), replacing the single 6-digit QUESTIONNAIRE_NO.
  - REGION_CODE value set is RESTRICTED to the 6 household-survey regions
    (III, V, NIR, VII, X, BARMM) per Sample Distribution 2026-05-12 — the
    config-driven F4 region whitelist.
  - F4 -> F3 linkage is a dedicated `F4_PARENT_F3_CASE_SEQ` item (length 3) in
    the household geo record.
  - Household GPS lives in its own capture record (`REC_HOUSEHOLD_CAPTURE`),
    consistent with F1/F3, instead of being embedded in the geo record.

Question content (Q1-Q202, sections A-Q) is ported verbatim from the
Apr 20 2026 Annex F4 questionnaire build. Item names are unchanged so
F4-Skip-Logic-and-Validations.md references stay valid.
...
# Make the v1.0 shared package importable.
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from shared.cspro_helpers import (
    numeric, alpha, yes_no, yes_no_dk, yes_no_na,
    select_one, select_all, record,
    build_field_control, build_dictionary, write_dcf,
)
from shared.case_id import build_id_block
from shared.psgc import load_region_value_set
from shared.gps_photo import gps_fields, photo_block


# F4 runs only in the 6 household-survey regions (Sample Distribution
# 2026-05-12): III, V, NIR, VII, X, BARMM. PSGC 2-digit region codes.
F4_REGION_WHITELIST = ["03", "05", "07", "10", "18", "19"]


# ============================================================
# FIELD CONTROL — shared + HH listing reference
# ============================================================

def build_f4_field_control():
    extra = [
        # Listing is manual / paper-based in v1.0 — this is the reference
        # number back to the paper household-listing line.
        numeric("HH_LISTING_NO", "Household Listing Reference Number",
...
# GEOGRAPHIC RECORD
# ============================================================
# Region/province/city are case-ID items. This record carries the household
# barangay, the F4->F3 parent linkage, and the address. Household GPS is a
# separate capture record (REC_HOUSEHOLD_CAPTURE), consistent with F1/F3.

def build_household_geo():
    items = [
        numeric("CLASSIFICATION", "Classification (catchment facility)", length=1,
                value_set_options=[("UHC IS", "1"), ("Non-UHC IS", "2")]),
        numeric("BARANGAY_CODE",
                "Barangay code (PSA 1Q 2026 PSGC pos 8-10)",
                length=3, zero_fill=True),
        numeric("F4_PARENT_F3_CASE_SEQ",
                "Parent F3 patient case sequence (F4->F3 linkage)",
                length=3, zero_fill=True),
...
def build_f4_household_capture():
    """GPS metadata for the household (off-form; wired in phase 4)."""
    return record("REC_HOUSEHOLD_CAPTURE", "Household GPS Capture", "Y",
                  items=gps_fields(prefix="HH_"))


def build_f4_case_verification():
    """Single verification photo per household case."""
    return record("REC_CASE_VERIFICATION", "Case Verification Photo", "Z",
                  items=photo_block(prefix=""))
...
# ============================================================
# Section C. Household Roster and Characteristics (Q30-Q50)
...
# convention). CSPro value codes can't carry a minus sign, so it is stored as
# "55". Source "88" ("Other (Specify)") is kept literally. Flag for the F4
# logic-pass spec.
...
def build_section_c():
    ...
    # -55 (source) stored as "55" — see module note above.
    YN_DK55 = [
        ("Yes",          "01"),
        ("No",           "02"),
        ("I don't know", "55"),
...
def build_section_c_gate():
    """Q47 HH-level gate for the private-insurance sub-roster — separate
    non-repeating record (type 'T') so the A-S letter sequence stays stable."""
    items = [
...
# F70 (GAMOT) and similar awareness items (identical 8-option list).
# ============================================================
F4_INFO_SOURCE = [
    ("News",                   "1"),
    ("Legislation",            "2"),
...
# ============================================================
# Section D. Awareness on Universal Health Care (UHC) — Q51-Q53
# ============================================================
...
# ============================================================
# Section E. YAKAP/Konsulta Awareness — Q54-Q56
# ============================================================
...
# ============================================================
# Section F. BUCAS Awareness and Utilization — Q57-Q61
# ============================================================
...
# ============================================================
# Section H. PhilHealth Registration and Health Insurance — Q79-Q88
# Respondent-level (non-repeating); gated by the respondent's Q45.
# ============================================================
...
# ============================================================
# Section J. Household Members' Health-Seeking Behavior and Outcomes
#            (Q101-Q107) — respondent-level (non-repeating)
# ============================================================
...
def build_section_j():
    ...
    # O02 text compressed — the verbatim form overflows CSPro's 255-char label
    # cap once select_all prepends the Q107 stem. Same compression as F3 Q87.
    Q107_ACTIONS = [
        ("Visited other healthcare facility",                                                                                  "1"),
        ("Sought alternative care (non-formal care — e.g. reflexology, acupuncture, massage therapy, herbal medicine)",         "2"),
...
# 6-option satisfaction scale with Not applicable (Q118, Q125).
SATISFACTION_6PT_NA = [
    ("Very Satisfied",                     "1"),
    ("Satisfied",                          "2"),
...
def build_section_k():
    # O06 text compressed — verbatim form overflows the 255-char label cap once
    # select_all prepends the Q109 stem. Same compression as F3 Q163.
    Q109_TYPE = [
        ...
        ("Special therapy visits (rehabilitation — e.g. occupational, physical, and speech/language therapy, psychosocial rehab, prosthetics/orthotics)", "06"),
...
# ============================================================
# Sections L/M shared value sets (NBB, ZBB, MAIFIP)
# ============================================================
...
# Source order preserved verbatim from Apr 20 Q128/Q134.
...
# ============================================================
# Section M. Zero Balance Billing (ZBB) Awareness + MAIFIP +
...
# Each line item emits the SHA triplet: consumed Y/N + purchased PHP +
# in-kind PHP. Sub-total rows (Q157, Q177, Q182, Q185) are CAPI-computed
# single numerics marked [DO NOT ASK] in source.
# ============================================================
...
def build_section_n():
    ...
    # Q178 label compressed — verbatim form (with the BCG/tetanus parenthetical)
    # overflowed the 255-char cap once the in-kind suffix is appended.
    items.extend(_expenditure_item(
        "Q178_PREVENTIVE",
        "178. Preventive services such as immunization/vaccinations and other preventive services. Exclude the cost of the vaccine itself."))
    items.extend(_expenditure_item(
        "Q179_DIAGNOSTIC",
...
# ============================================================
# Section P. Financial Risk Protection: Incidence of Reduced/Delayed Care
#            (Q197-Q199)
# ============================================================
...
# ============================================================
# Section Q. Anxiety about Household Finances (Q200-Q202)
# ============================================================
...
def build_id_items():
    """The 12-digit RR-PP-MMM-FF-CCC level ID block. REGION_CODE value set is
    RESTRICTED to the 6 F4 household-survey regions (the region whitelist)."""
    id_items = build_id_block("Household Survey")
    region_vs = id_items[0]["valueSets"][0]
    region_vs["values"] = [
        {"labels": [{"text": name}], "pairs": [{"value": code}]}
        for name, code in load_region_value_set(whitelist=F4_REGION_WHITELIST)
    ]
    return id_items


def build_dictionary_f4():
    records = [
    ...
    return build_dictionary("HOUSEHOLDSURVEY_DICT", "HouseholdSurvey",
                            build_id_items(), records)


def main():
    out_path = Path(__file__).parent / "HouseholdSurvey.dcf"
    write_dcf(build_dictionary_f4(), out_path)
