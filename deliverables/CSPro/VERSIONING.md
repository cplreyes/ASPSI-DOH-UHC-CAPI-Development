...
The full per-instrument version history lives in **`RELEASE-NOTES.md`** — GENERATED by
`automation/release_notes.py` from the git history of `versions.json`, and regenerated
automatically on every `stamp_version.py bump/set`. One entry per release, newest first.

Do **not** hand-maintain a table here (this section used to hold one; it drifted ~40
releases behind and was retired 2026-08-19 — its notes were folded into
`automation/release-notes-extra.json`). To add a human note for a release:
