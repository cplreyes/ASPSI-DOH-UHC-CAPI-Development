...
- [ ] **E3-F2-PWA-DESIGN-006** Sweep remaining raw Tailwind colors in `PendingCount.tsx` / `BroadcastBanner.tsx` / `SectionTree.tsx` / ReviewSection `warn` severity. Gated on extending `tailwind.config.ts` `theme.colors` with a `warning` token from `--warning`. `status::todo` `priority::low` `estimate::2h`
