...
- [ ] **E3-F2-PWA-DESIGN-001** PR #37 — token migration (`src/index.css` HSL rewrite to Verde Manual; `--radius` 0.5rem→0.25rem; manifest `theme_color` `#0f766e`→`#006B3F`). Validated: typecheck/lint/prettier clean; vitest 307/307; dev-server visual smoke confirmed. **Awaiting review/merge.** `status::review` `priority::high`
- [ ] **E3-F2-PWA-DESIGN-002** PR #38 — fonts via Bunny CDN with workbox `runtimeCaching` for offline use (Newsreader + Public Sans Variable + JetBrains Mono; Tailwind `font-{serif,sans,mono}` mapped). Validated: same as #37 + `document.fonts` API confirmed Public Sans loaded. **Awaiting review/merge.** `status::review` `priority::high`
- [ ] **E3-F2-PWA-DESIGN-003** PR 3 — layout & components refactor (`<Question>` 88px gutter + JetBrains Mono margin number; replace `<ProgressBar>` with typographic ledger; kill `border rounded shadow` card patterns across `<MultiSectionForm>` / `<EnrollmentScreen>` / `<ReviewSection>` / `<Section>`). **Branch deferred** until #37 + #38 merge so visual smoke runs against full Verde + fonts. `status::blocked-by-review` `priority::high` `estimate::4h`
