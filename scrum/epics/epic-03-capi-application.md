...
|---|---|---|
| F1 | CSPro CAPI | **Phase 1 build pipeline ~95% delivered 2026-05-08 on worktree `feature/uhc-survey-system-build`** (60 commits). End-to-end tablet proof-of-life proven: generator → F7-publish → CSWeb deploy → CSEntry sync → login_app → menu_app → F1 with partialSave. Last 5% blocked on `syncdata` external-dict CSPro 8.0 requirement (handoff memory `project_uhc_build_session_handoff_2026_05_08.md`). DCF at 12 records / 671 items; Designer sign-off DONE 2026-05-04 (E2-F1-010); E3-F1-001 FMF Section A superseded — FMF now generator-emitted; instrument-logic `.apc` wiring (skip-logic / validations / GPS / photo / PSGC cascade) deferred to Phase 2. |
...
### Phase 1 build pipeline (Plan 1 — landed 2026-05-08)

> **Plan 1:** End-to-end CSPro 8.0 build pipeline + tablet proof-of-life. Spec at `docs/superpowers/specs/2026-05-08-uhc-survey-system-build-design.md`; plan at `docs/superpowers/plans/2026-05-08-uhc-survey-system-build-phase-1.md`. Worktree `.claude/worktrees/uhc-survey-system-build` (60 commits). Phase 2 = PLF + F3 + F4_listing + F4 + supervisor menu + EA fence + daily audit Slack. Handoff memory: `project_uhc_build_session_handoff_2026_05_08.md`.

- [x] **E3-F1-080** Phase 1 — Python generator pipeline (canonical CSPro 8.0 source emitters for `.dcf`/`.fmf`/`.ent`/`.apc`/`.qsf`/`.mgf` via shared `cspro_helpers.py`) `status::done` `priority::critical` `actual::2026-05-08`
- [x] **E3-F1-081** Phase 1 — `build_all.py` orchestrator + env splice (`urls.yaml` → `.ent` userSettings via `splice_user_settings`; 3-env support dev/uat/prod) `status::done` `priority::high` `actual::2026-05-08`
- [x] **E3-F1-082** Phase 1 — `login_app` (101_login) chain entry point (Phase 1 simplification: no auth, hardcoded LOGIN_NAME / LOGIN_ROLE — to be replaced in Phase 2) `status::done` `priority::high` `actual::2026-05-08`
- [x] **E3-F1-083** Phase 1 — `menu_app` (106_menu) enumerator-only menu (Phase 1 simplification: no role check — `savesetting`/`loadsetting` cross-pen unreliable on Android, deferred to Phase 2) `status::done` `priority::high` `actual::2026-05-08`
- [x] **E3-F1-084** Phase 1 — F1 full generator chain (671 items / 12 records DCF; FMF emitted with canonical `[Form]` → `[Level]` → `[Group]` structure; ENT 14-key properties block; QSF UTF-8 BOM YAML; MGF UTF-8 BOM CSPro comment header). **Supersedes E3-F1-001** — FMF now generator-emitted; instrument-logic `.apc` wiring deferred to Phase 2. `status::done` `priority::critical` `actual::2026-05-08`
- [x] **E3-F1-085** Phase 1 — local CSWeb deploy proven (Wampserver64 + CSWeb 8.0.1 at `localhost/csweb8.0.1/`; Apache `Require all granted` for tablet LAN reach; F7-publish → Deploy Application wizard → .pen package upload; 4 users + 3 sync dicts registered) `status::done` `priority::high` `actual::2026-05-08`
- [x] **E3-F1-086** Phase 1 — CSEntry tablet sync chain proven (`192.168.1.168/csweb8.0.1/api/`, login chain `login_app → menu_app → F1` works on Android, F1 form renders, fields enterable, partialSave Suspend mid-case enabled). **Caveat:** sync (PUT) of completed F1 cases back to CSWeb tracked separately — see E3-F1-088. `status::done` `priority::critical` `actual::2026-05-08`
- [x] **E3-F1-087** Phase 1 — CSPro 8.0 syntax gotchas catalog (15+ undocumented gotchas: `config string` single identifier, `synchronize_data → syncdata`, `syncconnect(CSWeb, ...)` symbol, pff dot exec lowercase, `move to ITEM;` no parens, `elseif` one word, `protect()` vs `setattributes`, ASCII-only literals, MGF/QSF/FMF UTF-8 BOM, FMF group/level structure, DCF recordType marker rules, ENT 14-key properties block; runtime: CSDB binding for `syncdata`, external-dict requirement, savesetting unreliable on Android; Designer: F7 required, no headless `.ent → .pen`). Saved to handoff memory as Phase 2 / F3 / F4 reference. `status::done` `priority::medium` `actual::2026-05-08`
- [ ] **E3-F1-088** Phase 1 sync mechanic resolution — `syncdata(PUT, FACILITYHEADSURVEY_DICT)` requires external-dict reference + CSDB binding. Two paths: (A) separate "send" app w/ F1 dict as external + `dataFile` property in `.ent` or `setfile()` in code (~2h); (B) accept Phase 1 sync gap and ship Phase 1 substantially proven, revisit in Phase 2. Disposition decided in Sprint 004 retro / Sprint 005 plan. `status::todo` `priority::high` `estimate::2h` `scrum::sprint-005`

### Form layout & UX

- [x] **E3-F1-001** Create form file (`FacilityHeadSurvey.fmf`); lay out Section A (Identification & Cover Page) — **superseded 2026-05-08** by Phase 1 generator-emitted FMF (E3-F1-084); Section A formal layout no longer a discrete deliverable. `status::done` `priority::high` `actual::superseded by Phase 1 chain — FMF now generator-emitted`
- [x] **E3-F1-002** Lay out Sections B–H with grouped screens matching interviewer flow — **superseded 2026-05-08** by Phase 1 generator-emitted FMF (E3-F1-084); generator emits all sections via `form_layout_engine`. Phase 2 may revisit for human-review polish if needed. `status::done` `priority::high` `actual::superseded by Phase 1 chain`
- [x] **E3-F1-003** Assign capture types per field (radio, drop-down, number pad, date picker) to optimize tablet entry speed — **superseded 2026-05-08** by Phase 1 generator-emitted FMF (E3-F1-084); `form_layout_engine.pick_capture_type` sets these from spec metadata. `status::done` `priority::high` `actual::superseded by Phase 1 chain`

### Question text + language

- [x] **E3-F1-010** Populate English question text from questionnaire; use `~~item~~` fills for personalized text (facility name, respondent name) — **superseded 2026-05-08** by Phase 1 generator-emitted artifacts (E3-F1-084); `question_text_loader` extracts verbatim text from spec MD per `feedback_verbatim_questionnaire_labels` memory. `status::done` `priority::high` `actual::superseded by Phase 1 chain`
...
- [ ] **E3-F1-013** Externalize errmsg()/reenter strings to Messages tab with numbered IDs + `%s`/`%d`/`%f` format specifiers (per Khurshid 2022-06-22) `status::todo` `priority::high` `estimate::3h` `scrum::unscheduled`
...
- [ ] **E3-F1-014** Per-language value-set naming convention `<item>_vs1_<lang>` + three-stack language model wiring (dictionary + CAPI text + Messages tab) `status::todo` `priority::high` `estimate::4h` `scrum::unscheduled`
...
- [ ] **E3-F1-043b** PSGC cascade wiring — `onfocus` + `loadcase()` + `setvalueset()` per `PSGC-Cascade.apc`; external lookup dictionaries for REGION/PROVINCE_HUC/CITY_MUNICIPALITY/BARANGAY (F1 spec §4.15). PSGC value sets self-served from PSA 1Q 2026 + already wired into F1 DCF (per `project_aspsi_psgc_value_sets` memory) — unblocked 2026-05-08; awaits Phase 2 `.apc` wiring slot. `status::todo` `priority::high` `estimate::3h` `scrum::unscheduled`
...
- [x] **E3-F1-050** Partial save / resume behavior configured (what happens mid-interview) — **superseded 2026-05-08** by Phase 1 ENT properties block (E3-F1-084) enabling `partialSave: true`; verified on tablet (Suspend mid-case works). E3-F1-051 (OnStop + savepartial wiring) remains as discrete Phase 2 deliverable for the cross-instrument durable-resume pattern. `status::done` `priority::high` `actual::superseded by Phase 1 chain`
- [ ] **E3-F1-051** OnStop + savepartial wiring across F1/F3/F4 (durable resume on accidental tablet exit) `status::todo` `priority::high` `estimate::2h` `scrum::unscheduled`
...
### v2.0.1 Tier 1.5 — UAT R2 critical pull-ins *(✅ ALL SHIPPED OFF-SPRINT 2026-05-09)*

Pulled from `scrum/triage-2026-05-09-off-sprint-backlog.md` and **all 4 critical issues shipped Sat 2026-05-09 in the same off-sprint Saturday session**. Branch: `f2-admin-portal-v2.0.1` off `main@v2.0.0`. R3 testers re-verify in this round.

- [x] **E3-F2-PWA-R2-118-119** Section G + Section J data loss on back-nav. Root cause: `hasValue([])` returned `true` (multi-defaults init populated Q124/Q125 as `[]` in merged → `getSectionStatus` saw them as filled → auto-advance fired), plus `handleSectionValid` had no runtime completeness check. Fixed both layers: `hasValue` now mirrors `cross-field.ts isFilled` (handles empty arrays + whitespace); `handleSectionValid` gates advance on `getSectionStatus(SECTIONS[index], next) === 'complete'`. Effect propagates across all multi-conditional-required fields in B/C/D/E/J. `status::done` `actual::3h` `commit::484005e` `severity::critical`
- [x] **E3-F2-PWA-R2-114** Sections C/D/E role-gating regression. `shouldShowSection` only had a gate for Section G; C/D/E hit the catch-all `return true`. Added `SECTION_CDE_ROLES` (admin/doctor/PA/nurse/midwife/dentist/nutrition coordinator) and `SECTION_E_ROLES` (CDE + Pharmacist/Dispenser, since pharmacists need GAMOT half of E). 19 new tests cover all 5 role buckets + the 3 personas Shan tested. `status::done` `actual::2h` `commit::35acbdd` `severity::critical`
- [x] **E3-F2-PWA-R2-122** Double-submit dedup. Anchored `client_submission_id` on the draft id (already a UUID from `getOrCreateDraftId`) instead of generating a fresh `crypto.randomUUID()` per `submitDraft` call. Two-layer idempotency now: IDB `submissions.put()` upserts on primary key + server's `findExistingSubmission` catches duplicates. `status::done` `actual::1h` `commit::bc4ef93` `severity::critical`

### v2.0.1 Tier 1.5b — Bonus R2 fixes pre-built off-sprint *(✅ added 2026-05-09)*

Continued the same Saturday session past the 4 criticals. Originally Sprint 006 v2.0.2 candidates per triage report; now in v2.0.1 so R3 testers re-verify them this round.

- [x] **E3-F2-PWA-R2-110** Make Q9 Months required (spec change). Edited `spec/F2-Spec.md` line 88, dropped `, optional` from Month(s) annotation; regenerated `items.ts` + `schema.ts`. `status::done` `actual::30m` `commit::211a06b` `severity::medium`
- [x] **E3-F2-PWA-R2-108** Token "rejected" → "malformed" copy. Renamed `enrollment.tokenInvalid` in en + fil from "Token rejected. Contact ASPSI ops..." to "Token malformed. Contact ASPSI ops...". `status::done` `actual::20m` `commit::641e35a` `severity::low`
- [x] **E3-F2-PWA-R2-69** Admin login "Invalid credentials" copy. Renamed `errorMessageFor(E_AUTH_INVALID)` from "Username or password is incorrect." to "Invalid credentials." per Admin Portal R2 Tester Guide canonical phrasing. `status::done` `actual::20m` `commit::a953436` `severity::low`
- [x] **E3-F2-PWA-R2-109** Distinct offline copy on token verify. Added `enrollment.tokenOffline` translation key ("You're offline. Check your connection and retry."); `handleVerifyToken` maps `E_NETWORK` to it instead of falling through to `tokenInvalid`. `status::done` `actual::30m` `commit::70f8495` `severity::low`
- [x] **E3-F2-PWA-R2-117** Item-level role-gate for Section E1 (Q48–Q52). Pharmacists/Dispensers skip BUCAS questions; see only GAMOT half (E2 Q53–Q55). Implemented via item-level predicates in `skip-logic.ts` rather than structural section split (the latter would require `SECTIONS` array refactor + section-numbering updates). `status::done` `actual::1h` `commit::0c649ce` `severity::enhancement`
- [x] **E3-F2-PWA-R2-93** UTF-8 HMAC for non-ASCII filenames. `_gasHmacHex` now pre-encodes message + secret as UTF-8 bytes via `Utilities.newBlob(string).getBytes()` before calling `Utilities.computeHmacSha256Signature`. Worker uses TextEncoder (always UTF-8); pre-fix AS used the default Java charset (US-ASCII or ISO-8859-1) for string-to-bytes, so "Niño" gave 5 vs 4 bytes → HMAC mismatch → "Signature mismatch" toast. `status::done` `actual::1.5h` `commit::4354701` `severity::medium`
- [x] **E3-F2-PWA-R2-120-S.A2** Persist submitted state across refresh. Added `COMPLETED_CSID_KEY = 'f2_completed_csid'` to localStorage on submit success; App init effect short-circuits to `status='submitted'` if found. New "Start new survey" button on thank-you screen clears the flag. `status::done` `actual::1h` `commit::b41caeb` `severity::medium`

**Cascade-resolutions** (no code change — GH comments + labels only):

- [x] **R2-#112** B.E2 multi-select 0 selections — cascade-fixed by `484005e` (empty-array advance gate). The "None of the above missing as choice" sub-concern is a content/spec question for Kidd/Myra, not code.
- [x] **R2-#115** C.E2 force-nav to /section/c — resolved by `35acbdd` via different mechanism. SPA has no per-section URL routing; with #114 in place, non-CDE personas can't reach Section C through any path (SectionTree, URL, or programmatic nav).
- [x] **R2-#116** Q44 D auto-proceeds despite Q45–Q47 unanswered — cascade-fixed by `484005e` (Q45/46/47 are multi-conditional-required, exactly the empty-multi-as-filled bug surface).
