...
- [ ] **E4-PWA-013** Phase F — production cutover to Worker JWT proxy `status::blocked` `priority::critical` `estimate::1h` `scrum::unscheduled`
...
- [ ] **E4-PWA-014** Investigate Cloudflare Pages auto-deploy regression on **both** `staging` and `main` pushes (GitHub #34) `status::todo` `priority::high` `estimate::2h` `scrum::unscheduled`
...
- [ ] **E4-PWA-015** Lower Worker PBKDF2 default to 100k (Workers runtime cap) — GitHub #35 `status::todo` `priority::medium` `estimate::1h` `scrum::unscheduled`
...
- [ ] **E4-APRT-037** Concurrency tests (two-admin reissue race / bulk import + role edit / cron + PWA submit) `status::todo` `priority::high` `estimate::3h` `scrum::sprint-005`
