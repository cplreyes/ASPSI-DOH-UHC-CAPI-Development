...
- [ ] **E0-009** Issue-triage ritual — Day 1 of each sprint, audit Project #8 sprint slot framing + dispose of `fixed-pending-verify` + unscheduled Todo issues + post triage summary to `#capi-scrum` `status::todo` `priority::critical` `estimate::3h`
  - Sprint 005 first execution. Origin: Sprint 004 Retro Q2/Q4 — PR #136 (Sat 2026-05-09 inter-sprint) auto-closed 24 issues including 15 admin-portal items tagged `sprint-005` at merge time, raising a slot-framing question that didn't surface until archival. Per `project_sprint_005_resume_plan` Carl committed to triaging the ~64 open issues before starting Goal A/B work. Three sub-tasks: (1) audit current sprint slot — confirm framing of any items closed pre-sprint-open vs. rebill to inter-sprint activity; (2) triage `fixed-pending-verify` + unscheduled Todo (close / slot to a sprint / annotate `unscheduled-by-design` with rationale in issue body); (3) post triage summary to `#capi-scrum`. Recurring as a Day 1 ritual once first execution proves the format.
