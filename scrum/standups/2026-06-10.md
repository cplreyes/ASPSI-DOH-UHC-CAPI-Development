...
## Yesterday — since 2026-06-09

**deliverables/** (33 files)
- `deliverables/CSPro/CAPI-Logic-Open-Items-for-ASPSI.md`
- `deliverables/CSPro/CSEntry-Runtime-Verification.md`
- `deliverables/CSPro/CSWeb-Deploy-and-Android-Verification-Runbook.md`
...
- `deliverables/CSPro/F1/facility_lookup.dcf`
- `deliverables/CSPro/F1/generate_apc.py`
- `deliverables/CSPro/F1/psgc_barangay.dcf`
- `deliverables/CSPro/F1/psgc_city.dcf`
- `deliverables/CSPro/F1/psgc_province.dcf`
- _…and 21 more_
...
|---|----|------|--------|----------|----------|
| 1 | **E3-F1-001** | F1 Facility Head — CSPro Designer + CSEntry multi-language verify (EN + Cebuano/Bisaya/Hiligaynon/Waray/Bikol). Open `.dcf` (loads with declared languages), `.fmf` renders translated labels + language selection works (sync `.qsf` `languages:` if Designer flags mismatch), CSEntry toggles each language with skip logic intact, English base signed off. **Closes the five-sprint carry.** | in-progress | critical | 4h |
| 2 | **E3-F3-001** | F3 Patient — same Phase 3 verify (EN + Cebuano/Bisaya/Waray/Bikol). Eyeball-check the lower-coverage maps (F3 Bisaya 52%, Bicolano) on a question↔translation sample per the QC caveat. | in-progress | critical | 4h |
| 3 | **E3-F4-001** | F4 Household — same Phase 3 verify (EN + Cebuano/Bisaya/Waray/Bikol). | in-progress | critical | 4h |
| 4 | **E0-PSA-001** | Assemble the PSA bundle: F1/F3/F4 CAPI app artifacts (`.pff`/`.dcf`/`.fmf`/`.apc`) + the live F2 PWA reference + a per-language coverage/gap note (from `TRANSLATION-STATUS-2026-06-03.md` + the F2 status doc). Hand to ASPSI (Kidd/Myra) for the 6/12 gate as an in-chat go/no-go checklist. | todo | critical | 3h |
| 5 | **E4-PWA-014-clasp** | Wire `clasp` for the F2 PWA backend: `.clasp.json` + clean push dir from `dist/Code.gs` + `appsscript.json`, smoke-test `clasp push`, runbook. | todo | high | 2h |

## Blockers & Impediments

- **Blocked:** **E0-PSA-002** ASPSI translation-gap go/no-go: document fil/ilo (F1/F3/F4) + hil (F3/F4) as the missing-source gap (English-fallback in the bundle), tie to the 2026-06-01 + 2026-06-03 asks, and surface the gate decision (submit best-effort vs hold) for ASPSI.
- **At risk:** **E0-PSA-001** Assemble the PSA bundle: F1/F3/F4 CAPI app artifacts (`.pff`/`.dcf`/`.fmf`/`.apc`) + the live F2 PWA reference + a per-language coverage/gap note (from `TRANSLATION-STATUS-2026-06-03.md` + the F2 status doc). Hand to ASPSI (Kidd/Myra) for the 6/12 gate as an in-chat go/no-go checklist. — still `todo` past sprint midpoint
- **At risk:** **E4-PWA-014-clasp** Wire `clasp` for the F2 PWA backend: `.clasp.json` + clean push dir from `dist/Code.gs` + `appsscript.json`, smoke-test `clasp push`, runbook. — still `todo` past sprint midpoint

---

## Notes

_Auto-generated by `.claude/scripts/generate_standup.py` on 2026-06-10T08:30+08:00. Edit freely._
