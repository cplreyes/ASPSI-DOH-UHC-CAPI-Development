...
|---|----|------|--------|----------|----------|
| 1 | **E2-F1-010** | F1 DCF opened in CSPro Designer, full validation walkthrough (including case-control block), bug list closed or explicitly deferred, sign-off note recorded *(three-sprint carry — must not leave Sprint 004)* | todo | critical | 4h |
| 2 | **E3-F1-001** | F1 FMF Section A layout in CSPro Designer — generator skeleton ready; gated on E2-F1-010 sign-off | todo | high | 4h |
| 3 | **E0-010** | Define internal weekly status update format — partial-start at `deliverables/comms/_weekly-status-template.md`; close = format formalized | in-progress | high | 1h to close |
...
| 5 | **E4-APRT-039** | M10 sunset — rotate `ADMIN_PASSWORD_HASH` on staging Worker (`hash-admin-password.mjs` + `wrangler secret put`); offline backup; staging smoke test; open 7-day soak window with daily `wrangler tail --env staging` cadence (halt if 5xx >0.5% sustained 1h) | todo | critical | ~1.5h active + 7-day passive |
