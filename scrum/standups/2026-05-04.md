...
# Daily Standup — 2026-05-04, Monday

**Project:** UHC Survey Year 2 — CAPI Development
**Sprint:** 004 — *Close F1 sign-off (carry), complete Admin Portal QA, open M10 sunset soak* (2026-05-04 → 2026-05-08)
**Sprint Day:** 1 of 5
...
> **Goal B — F2 Admin Portal track:** Resume + complete cross-platform QA pass (E4-APRT-035, paused 2026-05-03 ~00:30 PHT at E1 Chrome G3); execute M10 sunset opening (E4-APRT-039) — rotate `ADMIN_PASSWORD_HASH`, smoke test, open the 7-day soak window with daily monitoring. Pull F3/F4 Designer validation into the week if Goal A clears mid-sprint.
...
- `deliverables/F2/PWA/worker/src/index.ts`
...
> Goal A is sequential (E2-F1-010 → E3-F1-001 once sign-off lands). Goal B runs parallel — open the M10 sunset soak window (E4-APRT-039) early in the week, run Admin Portal QA pass alongside (E4-APRT-035), then collapse onto Goal A. E0-010 is the quick close (~1h).

### Goal A — F1 Designer

| # | ID | Item | Status | Priority | Estimate |
|---|----|------|--------|----------|----------|
| 1 | **E2-F1-010** ✅ | F1 DCF Designer walkthrough complete; bug list clean — no deferrals; sign-off recorded; three-sprint carry **closed Day 1** | done | critical | ~confirmed via spot-check |
| 2 | **E3-F1-001** | F1 FMF Section A layout in CSPro Designer — generator skeleton ready; **UNBLOCKED** by E2-F1-010 sign-off | todo | high | 4h |
| 3 | **E0-010** ✅ | Define internal weekly status update format — template marked v1.0 stable; refinements from week-1 lessons folded back; **closed Day 1** | done | high | ~30m actual |

### Goal B — F2 Admin Portal

| # | ID | Item | Status | Priority | Estimate |
|---|----|------|--------|----------|----------|
| 4 | **E4-APRT-035** | Cross-platform QA pass — resume from paused state (E1 Chrome G3 onwards + E2-E5 not started; 3 findings logged for triage); PR #54 ready for merge once findings dispositioned | in-progress | critical | ~6h remaining |
| 5 | **E4-APRT-039** ✅ | M10 sunset — **closed Day 1**. Pivoted from rotation to deletion after smoke caught misframing (legacy `/admin/login` already 500'ing on staging; portal `/admin/api/login` checks F2_Users not the secret). `wrangler secret delete ADMIN_PASSWORD_HASH --env staging` clean; portal smoke green post-delete. Soak waived. Production-side equivalent rides E4-APRT-040 in Sprint 005. | done | critical | ~1h actual |

### Stretch (pull in if Goal A clears)

| # | ID | Item | Status | Priority | Estimate |
|---|----|------|--------|----------|----------|
| 6 | **E2-F3-010** | F3 DCF Designer validation pass | todo | medium | 3h |
| 7 | **E2-F4-010** | F4 DCF Designer validation pass | todo | medium | 3h |
| 8 | **E0-008** | Auto-standup retro-injection — extend generator to read prior sprint's Retro Q4 + Day 1 banner; also add no-work-since-last-run branch | todo | medium | 1h + 30m |
...
_Auto-generated by `.claude/scripts/generate_standup.py` on 2026-05-04T07:41+08:00. Edit freely._
