...
> [!warning] Sprint window exceeded — Day 39 of 5. Sprint 013 ran past its end date without being closed and rolled over, so this auto-standup is **provisional** — it reflects a sprint that should already be retired. Run `/scrum` to close Sprint 013 (Mode D retro + archive) and plan the next one. _Silence ≠ idle: an unreset sprint does not mean no work happened._

> [!warning] Scrum-state drift — `log.md` is 29.2 days newer than `sprint-current.md` (log last touched 2026-08-12, sprint board 2026-07-14). Work has been logged that the board hasn't absorbed, so its `status::` fields are behind reality. **Sync `sprint-current.md`** — fold the logged outcomes into the board — before trusting today's Sprint Board / Today columns. _(E0-SCRUM-SYNC canary.)_

---

## Stakeholder Context *(from Product Backlog)*

> Product Backlog last updated 2026-07-15 — 29 days stale. Consider regroom.
...
## Yesterday — gap window 2026-08-09 → 2026-08-12 (4 days)
...
_Auto-generated by `.claude/scripts/generate_standup.py` on 2026-08-13T10:25+08:00. Edit freely._
