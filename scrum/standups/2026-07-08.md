...
# Daily Standup — 2026-07-08, Wednesday
...
- `deliverables/CSWeb/csweb-dashboard-gen.py`
- `deliverables/CSWeb/f2-postgres-migration/F2-MySQL-Readmirror-Runbook.md`
- `deliverables/CSWeb/f2-postgres-migration/sync_f2_to_mysql.py`
...
## Intraday update (added ~19:00 — after the morning auto-standup)

**Goal A / E4-F2-ELESTIO advanced two big steps today:**
- **F2's data now lives on our prod server** — `csweb_f2` read-mirror deployed + cron'd (41 submissions, Sheet stays source of truth), and the **unified dashboard shows all four instruments** at csweb.asiansocial.org/docs/dashboard.html.
- **F2 gets the 12-digit QN** (Carl's call; facility-9 + HCW-seq-3, same anatomy as F1/F3/F4) — **code-complete full-stack** in the staging worktree: AS assignment + migration, QN bound into device-token claims, app threading, spec ×2, mirror column live. Tests: 202/238/518 green, `tsc -b --force` clean. Fully backward-compatible.

**Tomorrow / gates (Carl):** clasp push + `runAllMigrations()` + `/exec` redeploy · `wrangler deploy` · app staging→prod promote · rotate `HMAC_SECRET` (post-walk hygiene). Then the lane's remaining scope is the **serving** migration off Cloudflare/Google — which needs the box-vs-Elestio end-state decision.

## Notes

_Auto-generated by `.claude/scripts/generate_standup.py` on 2026-07-08T09:58+08:00. Edit freely._
