...
# Daily Standup — 2026-07-06, Monday

**Project:** UHC Survey Year 2 — CAPI Development
**Sprint:** 013 — **Ship `transform.py` (Epic 10 ETL variable extraction) end-to-end against the CSWeb breakout DBs and put the 47-partial/3-gap tabulation memo in front of ASPSI — while staying pretest-reactive** (pretest freeze stands, data-integrity exceptions only). _Locked Mon 2026-07-06._
**Sprint Day:** 1 of 5
...
## Yesterday — gap window 2026-07-03 → 2026-07-05 (3 days)

**(root)/**
- `index.md`
- `log.md`

**deliverables/** (78 files)
...
- `scrum/sprints/sprint-012.md`

**wiki/**
- `wiki/analyses/Analysis - DHS Benchmark vs ASPSI CAPI Gap Analysis.md`
- `wiki/sources/Source - DHS-Data-Processing CSPro CAPI Benchmark.md`

## Today (Plan)

| # | ID | Item | Status | Priority | Estimate |
|---|----|------|--------|----------|----------|
| 1 | **E10-ETL-EXTRACT** | — `transform.py`: per-instrument questionnaire-variable extraction from the CSWeb breakout DBs per codebook v0.4 (checkbox split-to-dummies; F3 pay-roster + F2 extracts; lowercased-DCF instrument vars + harmonized shared dims). Ends at real `.dta`/CSV feeding the proven Stata-12 tabulation do-files. **The single protected headline.** | todo | critical | 2d |
| 2 | **Tabulation decision memo → ASPSI** | — the 47-partial / 3-gap set (tables 1.6, 2.51, 4.2) written up as an in-chat go/no-go for Carl to relay: what each partial needs, what each gap means at the instrument. Companion to the headline; surfaces the ASPSI questions the extraction will otherwise stall on. | todo | high | 0.5d |
| 3 | **Pretest reactivity** | — freeze stands with **data-integrity exceptions only** (the weekend #830/#832/#833 deploys were exactly that). Field support (assignments/printed sheets ready) jumps the queue the day ASPSI confirms the pretest date. | reactive | high | — |
| 4 | **F4 Option C pilot** | — continues in the parallel session (v1.2.x); tracked here for visibility, not a Goal-A commitment. | in-progress (parallel) | medium | — |
| 5 | **Hub Designer compile + UAT R6 open** | — run the hub's fresh-Designer compile before its next deploy (F1/F3/F4 already cleared via the weekend `.csds` deploys); open the R6 artifacts the day ASPSI imports the accounts + sends real names. | blocked-on-ASPSI / todo | medium | — |
| 6 | **Support deliverables (rest)** | — governance (privacy/backup/retention) + training finalize (decks + Survey Manual screenshots, Kidd review). Advance only if Goal A is on track. | todo | medium | — |
...
_Auto-generated by `.claude/scripts/generate_standup.py` on 2026-07-06T11:56+08:00. Edit freely._
