...
**Inter-sprint, Sun 2026-04-26 (latest) — F2 PWA Verde Manual visual-identity migration COMPLETE on staging.** All three migration PRs merged today: #37 (Verde tokens, `47e1553`), #38 (fonts via Bunny CDN + SW runtime cache, `c1a97bb`), #39 (layout & components — marginal mono question ids, typographic ledger progress, kill-the-card refactor, Newsreader/JetBrains Mono applied to chrome, `4fd2d92`). Live staging now renders the full Field Manual experience: warm verde paper (`#F2F5EE`), DOH emerald primary (`#006B3F`), cherry-red destructive (`#B72020`, references the cross at the seal's center), Newsreader serif h1/h2, Public Sans body, JetBrains Mono on version subtitle + section eyebrows. All validation green: typecheck clean, lint 0 errors, vitest 308/308. End-to-end migration delivered in one session via `/gstack-design-consultation` + 3 focused PRs. `DESIGN.md` is the visual source of truth at `deliverables/F2/PWA/app/DESIGN.md`. **Two follow-ups remain open:** `E3-F2-PWA-DESIGN-004` (self-host fonts, gated on `pyftsubset`), `E3-F2-PWA-DESIGN-005` (refine hex from official DOH brand-book PDF, blocked-external on ASPSI), and a new low-priority `E3-F2-PWA-DESIGN-006` (sweep remaining raw Tailwind colors in `PendingCount` / `BroadcastBanner` / `SectionTree` / ReviewSection `warn` severity, gated on a `--warning` Tailwind binding).
