...
**Inter-sprint, Sun 2026-04-26 (latest) — F2 PWA Verde Manual visual-identity migration COMPLETE on staging.** All three migration PRs merged today: #37 (Verde tokens, `47e1553`), #38 (fonts via Bunny CDN + SW runtime cache, `c1a97bb`), #39 (layout & components — marginal mono question ids, typographic ledger progress, kill-the-card refactor, Newsreader/JetBrains Mono applied to chrome, `4fd2d92`). Plus #40 (`1458f29`) closing DESIGN-006 — added `--warning` + `--warning-foreground` to `index.css`, migrated `tailwind.config.ts` to alpha-aware HSL slot syntax, swept remaining raw Tailwind colors in `PendingCount` / `BroadcastBanner` / `MultiSectionForm` lock strip / `SectionTree` status badges / `ReviewSection` warn severity. Plus #41 (`7bfc28a`) polishing the chrome PR3 deferred — flat buttons (no shadows on shadcn primitive), hairline-edged modals (`SpecDriftOverlay`/`KillSwitchOverlay` paper rectangle on dark scrim, no `shadow-lg`), softer nav-arrow shadow (`shadow-md` → `shadow-sm`), Newsreader on modal titles. Live staging now renders the full Field Manual experience: warm verde paper (`#F2F5EE`), DOH emerald primary (`#006B3F`), cherry-red destructive (`#B72020`, references the cross at the seal's center), Newsreader serif h1/h2, Public Sans body, JetBrains Mono on version subtitle + section eyebrows. All validation green: typecheck clean, lint 0 errors, vitest 308/308, build clean. End-to-end migration delivered in one session via `/gstack-design-consultation` + 5 focused PRs. `DESIGN.md` is the visual source of truth at `deliverables/F2/PWA/app/DESIGN.md`. **Two follow-ups remain open:** `E3-F2-PWA-DESIGN-004` (self-host fonts, gated on `pyftsubset`), `E3-F2-PWA-DESIGN-005` (refine hex from official DOH brand-book PDF, blocked-external on ASPSI). DESIGN-006 closed `2026-04-26` (`status::done`).

**Inter-sprint, Sun 2026-04-26 (earlier) — F2 PWA design system codified; visual-identity migration kicked off (PRs #37 + #38).** `/gstack-design-consultation` produced [`deliverables/F2/PWA/app/DESIGN.md`](../deliverables/F2/PWA/app/DESIGN.md) — visual source of truth anchored on the memorable thing *"This is real software, not a government form."* Direction: **Verde Manual** — clinical stationery + utility, palette grounded in the DOH "Verde Vision / Berde para sa bayan" institutional brand. App-level CLAUDE.md added at `deliverables/F2/PWA/app/CLAUDE.md` to scope gstack to the F2 PWA workstream and cross-reference DESIGN.md.
...
**Visual-identity migration (Verde Manual) — COMPLETE on staging 2026-04-26.** All three migration PRs merged: #37 tokens, #38 fonts, #39 layout & components. Bundled into the v1.2.0 release window with the Round 3 UX enhancements (#16/#17/#18). See `deliverables/F2/PWA/app/DESIGN.md` for the full system. Two follow-ups deferred (`E3-F2-PWA-DESIGN-004` self-host fonts; `E3-F2-PWA-DESIGN-005` refine hex from official DOH brand-book) plus low-priority `E3-F2-PWA-DESIGN-006` (sweep remaining raw Tailwind colors).
...
| **3** | CAPI Application Development | **In Progress** (F2 **PWA in production at v1.1.1**, Rounds 1+2 closed, Round 3 v1.2.0 queued, Verde Manual visual-identity migration in flight via PRs #37/#38; F1 CSPro build pending Designer sign-off; F3/F4 Build-ready) | F1 `FacilityHeadSurvey.fmf` Section A layout (Sprint 003); F3/F4 build slots TBD; F2 Round 3 features + Verde Manual PR 3 (layout & components) when slot opens |
