...
**Inter-sprint, Sun 2026-04-26 (later) — F2 PWA design system codified; visual-identity migration kicked off (PRs #37 + #38).** `/gstack-design-consultation` produced [`deliverables/F2/PWA/app/DESIGN.md`](../deliverables/F2/PWA/app/DESIGN.md) — visual source of truth anchored on the memorable thing *"This is real software, not a government form."* Direction: **Verde Manual** — clinical stationery + utility, palette grounded in the DOH "Verde Vision / Berde para sa bayan" institutional brand (emerald `#006B3F` on pale verde paper `#F2F5EE`; cherry red `#B72020` references the cross at the seal's center). Type: Newsreader display + Public Sans Variable body + JetBrains Mono data — all OFL/Apache 2.0. Layout: hairline-divided, no cards, marginal mono question numbers in 88px gutter. App-level CLAUDE.md added at `deliverables/F2/PWA/app/CLAUDE.md` to scope gstack to the F2 PWA workstream and cross-reference DESIGN.md. **Migration plan: 3 small PRs.** PR #37 (Verde tokens — `src/index.css` HSL rewrite, `--radius` 0.5rem→0.25rem, manifest `theme_color` `#0f766e`→`#006B3F`) and PR #38 (fonts via Bunny CDN with workbox `runtimeCaching` for offline use) are open and validated (typecheck clean, lint 0 errors, vitest 307/307, dev-server visual smoke confirmed). PR 3 (layout & components — `<Question>` 88px gutter, `<ProgressBar>` → typographic ledger, kill-the-card refactor across `<MultiSectionForm>` / `<EnrollmentScreen>` / `<ReviewSection>`) deferred to a focused session **after #37 + #38 merge** so its visual smoke runs against the full Verde + fonts experience. Two caveats logged in DESIGN.md: (a) hex values are best-fit approximations of the visible DOH seal + documented Verde Vision tint `#e7efe7` — refine when official brand-book PDF (DO 2020-0011, Verde Vision 2023+) is available; (b) self-hosted woff2 in `/public/fonts/` is the long-term target, currently CDN-loaded via Bunny pending `pyftsubset` tooling for proper Latin Extended subsetting.
...
**Visual-identity migration (Verde Manual)** — separate workstream from Round 3 UX, but bundled into the same v1.2.0 release window. PRs #37 (tokens) + #38 (fonts) open as of 2026-04-26; PR 3 (layout & components — 88px gutter, ledger progress, kill-the-card) follows after #37 + #38 merge. See `deliverables/F2/PWA/app/DESIGN.md` for the full system; PRs are independent and can merge in either order.
