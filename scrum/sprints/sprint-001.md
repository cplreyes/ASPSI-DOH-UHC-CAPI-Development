...
- **F2-Build-Handoff drafted Day 3.** `deliverables/F2/F2-Build-Handoff.md` — 3-phase recipe (Build / Seed / Test) covering paste-into-script.google.com, `FacilityMasterList` stub with 3 test rows across facility-type splits, Shan-facing test cases per role bucket + ZBB/NBB split + terminal branch, response-Sheet POST-rule validation checklist, and symptom→source-doc bug routing table per the forward-only rule. F2 is now handoff-ready; Carl can paste + run today, Shan can start testing as soon as links are generated.
- **E3-F2-GF-001..008 drafted Day 3.** Apps Script bundle at `deliverables/F2/apps-script/` — `Code.gs` (entry + triggers), `Spec.gs` (all 114 items as structured JS data, 35 sections, verbatim labels), `FormBuilder.gs` (two-pass materialization + routing wiring), `OnSubmit.gs` (20 POST rules from F2-Cross-Field.md), `Reminders.gs` (daily Day 1/2/3 nudge reader), `Links.gs` (prefilled URL generator from FacilityMasterList), `Routing.gs` (placeholder), `README.md` (paste-into-script.google.com recipe). Ready for Carl to paste into a new Apps Script project under the ASPSI project mailbox and run `buildForm()`. Covers E3-F2-GF-001 through -008 and the Section-based skip logic wiring from -003. -009 (staff encoder), -010 (paper mirror), -011 (Filipino translations), -012 (desk test), -013 (3-day dry-run), -014 (Shan QA pass), -015 (gate decision) still open.
- **E2-F2-015 + E2-F2-016 also closed Day 3.** Validation inventory at `deliverables/F2/F2-Validation.md` (4 hard-required fields, everything else optional, Q103 lifted from grid, Q68–Q72 as standalone items) and cross-field rules at `deliverables/F2/F2-Cross-Field.md` (20 POST rules across 6 groups, targeting Apps Script `onFormSubmit` + nightly `cleanSheet()`). F2 design stack (013/014/015/016) now fully drafted — unblocks the E3-F2-GF Apps Script generator. Per the forward-only sign-off rule, proceeding directly to build without waiting on Shan's review of the design bundle; test bugs will route back to the owning source doc.
- **Inbound comms — DOH-PMSMD review window set.** PMSMD acknowledged receipt of the matrices + Word-version data collection tools (Juvy sent Apr 14 PM) and **requested until Mon Apr 20 to return feedback**. Thread: `Re: Update on the Conduct of the UHC Year 2 Survey`, from PMSMD Apr 15 11:54 AM, to Juvy + ASPSI team (Carl cc'd on UP mail). This sets an external checkpoint: **DOH feedback lands Apr 20, Tranche 2 (40%) is due Apr 24** — 4-day turnaround if revisions are requested. Added to D2/D3 exposure watchlist under E0-032. No action required from Carl until Apr 20; morning of Apr 20 is the right time to triage any requested revisions against the F1/F2 build state.
- **Inbound comms — Apr 13 LSS/Team meeting notes circulated.** ASPSI (`aspsi.doh.uhc.survey2@gmail.com`) sent `Team Meeting Notes April 13, 2026` Apr 15 5:23 PM with attached notes + slides (supersedes an earlier send). **Ingested Apr 15 PM** — full source at `wiki/sources/Source - ASPSI Team Meeting 2026-04-13.md`. **Finding: the meeting did NOT discuss any of the 6 open F1 items.** Agenda was process-focused (lessons learned, comms lines, D2/D3/D4 tasking). **E2-F1-009b stays BLOCKED.** Carl must escalate to Dr Myra / Dr Claro to convene a technically-scoped LSS session. Also established Team Communication Protocol (new concept page): Carl is NOT an authorized DOH-facing sender — route everything through Juvy / Dr. Claro / Dr. Paunlagui. Other findings: Carl's "CAPI development + bench-testing" D2 scope formally confirmed; DOH status cadence shifting weekly → bi-monthly (affects E0-010); Shan name contradiction flagged on entity page pending Carl's confirmation.
- **Inbound comms — Apr 13 Indicator Matrix Google Doc shared** by `spprt.aspsi.doh.uhc.survey2@gmail.com` (`Indicator Matrix_April 13`). Recipient list includes Carl + ASPSI team — this is likely the source artifact behind the matrices PMSMD is reviewing. No immediate action; flagging the provenance link in case PMSMD feedback points back at specific indicator rows.
