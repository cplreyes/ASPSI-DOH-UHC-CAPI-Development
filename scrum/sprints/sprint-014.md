...
closed: 2026-08-24 — CLOSED LATE at Day 32 of 5, 31 calendar days past the window
---
...
## What actually happened after the window closed (2026-07-25 → 2026-08-22)

> Reconstructed at the late close from git history, the UAT register, and `log.md`.
> **This work was real, shipped, and largely excellent — it simply happened with no sprint
> container around it.** Recorded here so the month is not a hole in the record.

| Window | What shipped |
|---|---|
| **Jul 25 – Aug 4** | Pretest close-out; fix-evidence backfill; the pretest→rollout work synced across CSPro, F2, CSWeb and docs. |
| **Aug 5 – 6** | F2 PWA v2.1.0 (spec-r8); F1 v1.2.4 batch — 17 issues; F4 v1.5.x series (Q17/Q45/Q52/Q117/Q141). |
| **Aug 8 – 10** | **CSWeb console unification** — one shell, one URL space, shared portal partial, permission-accurate nav. F2 Admin Facilities page brought onto main. F1 v1.3.1 / F3 v1.4.1 / F4 v1.5.9 deployed. |
| **Aug 11 – 13** | Cover logo strip (#1190, reproducible via `compose_cover_logos.py`); F3 review wave (8 issues); triage waves cleared. |
| **Aug 14 – 18** | **Bicolano translation wave** — ~30 tickets, plus two systemic defects: a directive printing twice across 6 locales (365 values) and 257 stored translations carrying instructions/answer codes inside the translated text. |
| **Aug 17 – 20** | **The Aug-17 instrument migration** — all four instruments rebuilt against the new questionnaire set, F1 and F2 renumbered. F3 section order restored to the printed sequence. **UAT Round 7 opened Aug 19**; 23 findings closed Aug 20. |
| **Aug 20 – 21** | **PSA submission set frozen** — F1 v3.1.5 · F2 v3.0.0 · F3 v6.0.2 · F4 v3.1.3, tag `capi-psa-2026-08-20`, sources tagged + packages archived. **Submission sent Aug 21.** DEV channel opened so no later build can be mistaken for the submitted one. |

**Net:** the engagement moved from *pretest complete* to *submitted to PSA* during a sprint that
had formally ended on 2026-07-24.

## Retrospective — Sprint 014

> Filled at the LATE close, 2026-08-24 (Day 32 of 5). Written honestly rather than tidily —
> a retro that flatters the month would hide the one finding worth keeping.

### 1. Did the sprint goal land? (yes / partial / no — one line why)

**Partial on the work; NO on the container.** The pretest goal itself was met — field support ran
same-day, the pretest completed, and the record fed forward. But Sprint 014 was never closed: it
ran **32 days against a 5-day window**, and roughly 4½ weeks of major delivery (console
unification, the Bicolano translation wave, the Aug-17 migration, the PSA submission) happened
with **no sprint around it at all**.

### 2. What surprised me? (process, not work — max 3 bullets)

- **The rule written to prevent exactly this was ignored.** S013's retro produced "close-or-roll
  on the first *window exceeded* warning", and it was baked into S014's own Definition of Done.
  The warning then fired on every standup for four weeks and nothing happened. A rule with no
  enforcement point is a note, not a rule.
- **The close ritual was the one with no tooling.** `/scrum` implements Mode B and C (kickoff and
  mid-sprint) but Modes A and D — *planning* and *sprint close* — have been stubbed
  "coming in Pass 2" since April. The one ceremony that ends a sprint had no button, so it
  never got pressed. The gap was structural, not personal.
- **Nothing broke, and that was the problem.** Output stayed high the entire month, so the
  failing container produced no pain and nothing forced the issue. Process decay is invisible
  when delivery is healthy — which is precisely when it needs an external trigger.

### 3. Deadline exposure check — D2 / D3 / Tranche slip days this sprint

_Informational only (out of Data Programmer scope per CSA D1–D6)._ Material change: the PSA
submission went out **2026-08-21**, and training is now dated — **week of September 07**.

### 4. One thing to change in Sprint 015

**Give the close ritual a button, and make the warning a hard stop.** Implement `/scrum` Mode D
(close + archive + reset) and Mode A (plan), so closing a sprint costs one command instead of an
afternoon. Until that exists, the standing rule is: **the first standup showing "window exceeded"
gets the sprint closed that morning, before any other work.** S013 made this rule; S014 proved
that stating it is not enough.
