...
- [[1_Projects/ASPSI-DOH-CAPI-CSPro-Development/wiki/concepts/F4 Listing - Hybrid Frame Model]] — **(adopted 2026-05-12)** Build-time resolution of the Survey Manual Protocol V2 line 1199-1201 `{.mark}` (fresh-vs-existing listing question). `LISTING_SOURCE` field on `REC_LISTING_CONTROL` selects 1 = captain-supplied (primary) or 2 = fresh door-to-door (fallback); both modes feed the same `REC_LISTING_BRGY_FRAME` schema. CAPI carries this until Myra's Survey Manual edit pass resolves the Protocol V2 question. Implements Carl's Q1-Q8 design sign-off for `113_F4_listing` (LISTING_SOURCE / REPLACEMENT_RESERVES=10 / 20-digit ID block reuse / 999 occ cap / MOBILE alpha-11 / GPS-only / "F4 Household Listing" menu / BARANGAY_CODE-seeded PRNG).
