...
- [[1_Projects/ASPSI-DOH-CAPI-CSPro-Development/wiki/sources/Source - DHS-Data-Processing CSPro CAPI Benchmark]] — **The DHS Program's standard CSPro suite** (836 files, 238 MB mirror in `raw/psa-cspro-capi-reference/github/`), the system behind PSA's NDHS 2022. 4-role menu-driven micro-app architecture (DCMenu/SupMenu/BioMenu/CentralMenu, 24-app registry, ~26 aux dicts, runtime .apc Library); template-adaptation dev model (`!!!` markers, DPP-02); integer-counter menu-integrated upgrade machinery; FCT paradata field-check tables; CC81NEW 8.1-era consolidation. Ingested 2026-07-03 as the professional benchmark for our F1/F3/F4 + Supervisor Hub build.
...
- [[1_Projects/ASPSI-DOH-CAPI-CSPro-Development/wiki/analyses/Analysis - DHS Benchmark vs ASPSI CAPI Gap Analysis]] — **Comparison & gap analysis vs the DHS standard suite** (2026-07-03): 18-dimension side-by-side matrix; gap register G1–G9 (headline: G1 supervisor field-check tables on paradata, G7 enable paradata at pretest, G2 hub update-check, G3 app-list hygiene toggle); reverse gaps validating generators/SemVer/automated deploy; **§4 CC81NEW refactor deep-read → techniques T1–T7** (publishdate() build stamp, unified case list, runtime per-case PFF launch, BT device-name pairing, paradata concat→CSWeb logistics, CleanIndex/fixdata self-repair, on-device styled HTML reports); **§5 Khurshid cross-benchmark → K1–K5** (his 3 headline mechanisms already adopted by the hub; remaining = encrypted roster, device-bound login, SUPERVISOR_ID + Excel roster import, per-login files); **§6 per-CSPro-tool structure review → refactors R1–R3** (R1a hand-procs out of Python heredocs, R1b external-logic library per the official 8.x CAPI Census pattern, R2 numbered multi-language .mgf messages vs 236 inline English errmsg literals, R3 FMF normalization); timeline-aware recommendations (pretest freeze → Aug training → Sep rollout → roster-delivery items). §6 status blocks: **Option B data layer + R1a BUILT, then R2 BUILT + R3 substantially closed (both 2026-07-03)** — remaining: R1b (Aug), F1 fmf-generator fold, K1 (Sep). Companion diagram: `wiki/analyses/DHS-vs-ASPSI-comparison.png` (.excalidraw source alongside).
