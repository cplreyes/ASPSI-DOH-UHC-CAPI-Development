# CAPI Surveys (F1 / F3 / F4) — UAT Round 4 · End-to-End Field-Day Dry-Run — CSWeb Monitoring Companion Guide
...
**Companion guides:** the three instrument tester guides — `docs/F1-CSEntry-Tester-Install-and-Test-Guide-2026-06.md`, `docs/F3-…`, `docs/F4-…` (the enumerator side that *generates* the data you monitor)
...
1. **Deploy the R4 build ×3 to CSWeb** (F1 + F3 + F4) — the standard publish-and-deploy handoff. Confirm each app downloads cleanly on a real device once.
...
3. **Create the 3 Slack channels** `#f1-uat` / `#f3-uat` / `#f4-uat`; add testers + STLs + the monitor; pin the relevant guide in each.
4. **Confirm the GitHub feedback form is live** (`capi_uat_feedback.yml`), the label `from-uat-round-4-capi-2026-06` exists, and the **tracking issues #368 (F1) · #369 (F3) · #370 (F4)** are open.
...
| **Bug-filing label** | `from-uat-round-4-capi-2026-06` |
| **Slack channels** | `#f1-uat` · `#f3-uat` · `#f4-uat` |
| **Tracking issues** | F1 [#368](https://github.com/cplreyes/ASPSI-DOH-UHC-CAPI-Development/issues/368) · F3 [#369](https://github.com/cplreyes/ASPSI-DOH-UHC-CAPI-Development/issues/369) · F4 [#370](https://github.com/cplreyes/ASPSI-DOH-UHC-CAPI-Development/issues/370) |
...
CSWeb console (csweb.asiansocial.org/csweb/)
  Applications      → FacilityHeadSurvey · PatientSurvey · HouseholdSurvey (R4 deploys)
  ...
  Users             → setest (fallback) + R4 users: alytest / aidan (Field Sync) · shan + kidd + marriz (Administrator) · admin/cplreyes (system)
```
...
- **5B.4 (map pins):** On the **Map Report**, confirm cases **with a real outdoor GPS fix** plot a pin (clicking opens the case). **Desk/indoor cases will have blank GPS and no pin — that's expected**; note which of the day's cases were outdoor vs desk so the blanks are explained, not mysterious.
- **5B.5 (monitor's verdict):** Narrate (in a finding / note) anything that would slow real-time monitoring on a real field day — refresh lag, an unclear or wrong count, a case that broke out with missing fields, a view you wish you had. **This is the most valuable output of the dry-run from the monitoring seat.**
...
- **5D.3 (content cross-check):** For one case per instrument, open **View case** and confirm the answers match the tester's notes end-to-end — including the R4 features (photo present on Completed / absent on Refused; F3 branch; F4 member count).
...
Pick the **Instrument** the case belongs to (F1/F3/F4), and for the **scenario** use `Sync to the server` or `General observation` (monitor-side findings), then describe it. Auto-label: `from-uat-round-4-capi-2026-06`. By-hand shape:

```
**Round 4 (dry-run) — monitor step:** 5A / 5B.x / 5C.x / 5D.x
...
Tag the instrument's tracking issue (**#368 / #369 / #370**). Coordinator triages daily.
