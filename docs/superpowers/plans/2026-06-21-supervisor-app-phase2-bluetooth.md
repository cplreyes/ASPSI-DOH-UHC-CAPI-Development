...
>
> **UPDATE 2026-06-27** — B6/B7 Bluetooth case exchange is **device-confirmed** + the B7 relay is now **real logic**. Post-confirmation refinements from the Arshad Khurshid "101 - Applications" ingest (the grouped **menu interface**, `\|type=None`, HTMLDialogs/C8 report, roster hardening, and the **source folder-structure refactor**) are planned separately in **`2026-06-27-supervisor-hub-khurshid-adoptions.md`**.
...
- **C7 (on-device map, N3) = PASS (2026-06-25)** — on-device EA maps are a NATIVE CSPro 8 capability (NOT the `view()`-of-KML/HTML route this plan originally sketched). The `Map` API (`Map m; m.addMarker(lat,lon); m.show();`) compiles clean (probe `spikes/MapSpike`, `MAPSPIKE_compile.png`) and is documented to render markers on Android/CSEntry with **offline MBTiles base maps** via `setBaseMap()` / PFF `BaseMap=`. Verdict + N3 build path in `spikes/C7-map-spike.md`. The only N3 work item is provisioning an **MBTiles** offline base map (HOT-OSM/QGIS export, clipped + zoom-capped). EA *boundary* polygon unconfirmed → render as marker points. **B9 unblocked.**
...
### Task B9: N3 — on-device EA map wiring  *(C7 = PASS; the one remaining item is the MBTiles file)*
**Files:** `build_hub_apps.py` (the "View EA on Map" menu action — `Map`/`addMarker`/`setBaseMap`/`show`), `map/` (the MBTiles offline base map + provisioning notes).
**What:** (1) **Provision the MBTiles offline base map** — raster export (HOT-OSM / QGIS) clipped to the survey provinces, zoom-capped for size; ship with the deployment or `syncfile`. (2) Wire both roles' "View EA on Map": `Map m;` → add a marker per case/EA point (`m.addMarker(lat,lon)`) → `m.setBaseMap(<mbtiles>)` → `m.show()`; returns to the menu on close. EA boundary renders as marker points (polygon overlay unconfirmed — stretch goal). C7's documented native Map API means no `view()`/KML fallback is needed.
**Acceptance:** on the itel with **Wi-Fi OFF** and the MBTiles in place, the menu item renders the EA + case markers offline and returns to the menu (the online render is trivially covered by the default base map).
