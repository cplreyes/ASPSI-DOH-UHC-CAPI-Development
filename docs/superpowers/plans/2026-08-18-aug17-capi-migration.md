...
- [ ] **Step 2: Q18 income bands — VERIFY ONLY (struck 2026-08-18, Ruling R4)** — already landed pre-plan (#631, #1176: 11 bands + DK=90/RF=95, `Q18_BRACKET`/`Q18_INCOME_BRACKET`). Just confirm the built shape matches `normalized/F3-paper.csv` row qnum=18 verbatim (Tier-1 will also catch it); fix only actual text/code deltas.
...
- [ ] **Step 3b: Result-of-visit mapping (narrowed 2026-08-18, Ruling R4)** — the 6-code `ENUM_RESULT_OPTIONS_F3` list ALREADY matches the paper (verified Task 0.1); remaining work is only to verify/align the `DISPOSITION_PROCS` BREAKOFF→Result-of-Visit→CASE_DISPOSITION wiring against the paper's codes; Tier-1 `DISPOSITION_DIFF` and a Tier-2 matrix row per code prove it.
...
- [ ] **Step 3: Small deltas** — Q34 relationship code **13 Grandfather/Grandmother** appended (roster value set, append-only); Q202's COVID-19 option per paper; Q11↔Q40 education lists (REWRITTEN 2026-08-18, Ruling R5): the build's lists were deliberately UNIFIED per #1204 (the paper itself says Q40 mirrors Q11) — do NOT re-diverge them; verify the unified list matches the Aug-17 Q11 table, and if the paper's two tables still disagree with each other, keep unified and register `class=defect-fix` + ASPSI item.
...
- [ ] **Step 2b: Result-of-visit mapping (narrowed 2026-08-18, Ruling R4)** — `ENUM_RESULT_OPTIONS_F4` ALREADY exactly matches the paper's 4-code no-Refused list (verified Task 0.1); remaining work is only to verify/align `DISPOSITION_PROCS` wiring; Tier-1 `DISPOSITION_DIFF` + Tier-2 rows prove it.
...
- [ ] **Step 4: Version + deploy** — `stamp_version.py set F1 3.0.0` → `show` clean → compile + csentry_verify + gates → `auto_deploy.py F1 --deploy` (8 PSGC files ride per add_files; ⚠ CORRECTED 2026-08-19: facility_lookup must NOT ride — removed from the bundle 2026-06-10, its PRESENCE re-creates the Android "always loading" startup loop; treat it appearing in the driver output as a defect). Release note via `release_notes.py note F1 "…"` (⚠ CORRECTED 2026-08-19: the VERSIONING.md hand table was retired that day — do not hand-write rows). Commit (MAIN + WT) — `aug17: F1 v3.0.0 deployed`
