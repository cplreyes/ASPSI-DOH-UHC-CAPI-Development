...
R4 runs on **staging**, not production, by design:
...
5. **Create the round tracking issue** `[E6-PWA-NNN] Pretest Pilot Dry-Run (R4) — HCW + Admin, staging, v2.1.0` and the label **`from-pretest-dryrun-2026-06`** (color it distinctly). Drop the issue # into Section 1.
...
| **Bug-filing label** | `from-pretest-dryrun-2026-06` (coordinator creates before opening) |
| **Slack channel (blockers + daily check-in)** | `#f2-pwa-uat` on `aspsi-doh-uhc-survey2.slack.com` |
| **R4 tracking issue** | [#367](https://github.com/cplreyes/ASPSI-DOH-UHC-CAPI-Development/issues/367) — `[E6-PWA-367]` |
...
> If any 5C item is **missing, mis-worded, or misbehaving**, file it referencing the v2.1.0 issue if you can (e.g. `Regression of #310` for Q47). Verbatim wording mismatches are high-value — the survey goes to PSA on 2026-06-12.
...
```
**Round 4 (dry-run) step:** 5A.x / 5B.x / 5C.x / 5D.x / 5E.x
...
Apply the label `from-pretest-dryrun-2026-06` to every R4 issue and link it to the tracking issue. Coordinator triages daily.

---

## 7. Triage Cadence

- **Daily check-in:** Coordinator posts to `#f2-pwa-uat` daily 09:00 PHT — new R4 findings, criticals, overnight fixes.
...
- **R4 close:** Friday 2026-06-12 PM. Coordinator reconciles the tracking issue; open items dispositioned (fix-now / next-sprint / route-to-survey-team / won't-fix-with-rationale). Findings that are survey-design/wording are routed, not coded.

---

## 8. Why this round matters

Rounds 1–3 were lab passes — scripted scenarios on desktop browsers, then on real devices. They were right for finding defects. **Round 4 is the rehearsal before the real thing.** The pretest pilot is when actual HCWs at actual facilities meet this survey, days after the 2026-06-12 PSA gate. Before that, we want one honest run-through that proves:
...
If R4 runs clean, we walk into the pretest confident. If it doesn't, this is the last cheap place to find out. Thanks for testing — and treat it like the real day.
